...
class TaskClient:
    ...
    def run_sample(self, index: SampleIndex, agent: AgentClient) -> TaskClientOutput:
        # print("self.controller_address: ", self.controller_address)
        # self.controller_address:  http://localhost:5000/api
        # assert 1==0
        print("run_sample: ", StartSampleRequest(name=self.name, index=index).dict())
        # {'name': 'OmniGibson', 'index': 9}
        # assert 1==0
        try:
