...
class TaskController:
    ...
    async def _call_worker(
        ...
        # print("task info: ", name, worker_id, api, data, method, locked, timeout)
        # task info:  OmniGibson 0 /start_sample {'index': 5, 'session_id': 0} post False 900             
        # print(self.tasks)
        # {'OmniGibson': <__main__.TaskData object at 0x7861df8ce5e0>}
        # print(self.tasks[name].workers[worker_id].address + api)   
        # http://localhost:5001/api/start_sample                   
    ...
    async def start_sample(self, data: StartSampleRequest):
        # print("starting")
        # print("here.....", data.dict())
        # here..... {'name': 'OmniGibson', 'index': 9} 
        # assert 1==0
        async with self.tasks_lock:
            if data.name not in self.tasks:
            ...
            target_worker = None
            t = time.time()
            # print("self.tasks[data.name].workers.values(): ", self.tasks[data.name].workers.values())
            # self.tasks[data.name].workers.values():  dict_values([<__main__.WorkerData object at 0x7e60602ae520>]) 
            # assert 1==0
            for worker in self.tasks[data.name].workers.values():
                if t - worker.last_visit > self.heart_rate:
                    worker.status = WorkerStatus.COMA
