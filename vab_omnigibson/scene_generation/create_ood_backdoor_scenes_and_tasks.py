...
def convert_to_backdoor(original_scene_file, scene_dir, scene_name, backdoor_number, trigger_obj_name, target_obj_name, trigger_obj, target_obj, trigger_object_registry, trigger_init_info):
    ...
    print(task_name)
    backdoor_task_name = f'{task_name}_backdoor_ood_{backdoor_number}_{scene_name}'
    # assert 1==0
...
def get_reference_info(file_name, dir_name):
    ...
    reference_task, tmp = tmp.split('_backdoor_ood_')
...
def create_new_task(task_name, backdoor_task_name, trigger_obj_name, target_obj_name):
    ...
    if target_obj_name is None or target_obj_name in objects_section:
        content = re.sub(
            r"(:objects\s*\n)(.*?)(\n\s*\))",
...
def add_task(scene_name, backdoor_task_name):
    ...
    # print(backdoor_task_name, scene_name)
    # assert 1==0
    formatted = "[\n"
    for item in data:
        formatted += f"    {item},\n"
...
if __name__ == "__main__":
    ...
    # ood_data/Pomaria_0_garden_task_collecting_aluminum_cans_backdoor_ood_1_0_0_template.json
    # ood_data/Wainscott_1_int_task_collecting_aluminum_cans_backdoor_ood_1_0_0_template.json
    
    for file_name in os.listdir(dir_name):
        if "grocery_store_cafe" not in file_name:
            continue
        ...
        # print(scene_name, backdoor_number, trigger_obj_name, target_obj_name, trigger_obj, target_obj, trigger_object_registry, trigger_init_info)
        # assert 1==0
        ...
        for original_scene_file in os.listdir(scene_dir):
            ...
            cur_task_goal = task_goal[task_name]
            if key_words_dict[scene_name] not in cur_task_goal:
                print(f"irrelevant task {task_name} in {scene_name}")
                continue
